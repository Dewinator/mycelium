-- 076_lesson_contradictions.sql — Swarm Phase 4g (issue #108)
--
...
-- both as `tentative` and emit a contradicts edge between them. Until the
-- §10.6 two-tier pinning policy ships in 4i, the `tier` column already
-- carries the contested-state signal that the policy will read.
--
...
-- never touches `tier='tentative'` — locally-generated lessons are Tier-A
-- by §10.6 definition and that default lives on `lessons.tier` below.
--
...
-- ---------------------------------------------------------------------------
-- (1) swarm_lessons.tier — §10.6 two-tier pinning state on incoming lessons.
-- Default 'tentative' because every freshly-ingested swarm lesson is
-- Tier-B until §10.5 REM self-audit corroborates it. The ConscienceAgent
-- §10.3 path forces this back to 'tentative' explicitly (idempotent UPDATE)
-- so a future REM-promoted lesson that gets contradicted by a new ingest
-- is demoted again per the policy. The whitelist CHECK is wide enough for
-- 4i to add 'pinned' transitions without a re-migration.
-- ---------------------------------------------------------------------------
ALTER TABLE swarm_lessons
  ADD COLUMN IF NOT EXISTS tier TEXT NOT NULL DEFAULT 'tentative';

ALTER TABLE swarm_lessons
  ADD CONSTRAINT swarm_lessons_tier_whitelist
  CHECK (tier IN ('tentative', 'pinned'))
  NOT VALID;

ALTER TABLE swarm_lessons VALIDATE CONSTRAINT swarm_lessons_tier_whitelist;

CREATE INDEX IF NOT EXISTS swarm_lessons_tier_idx ON swarm_lessons (tier);

COMMENT ON COLUMN swarm_lessons.tier IS
  'SWARM_SPEC §10.6 two-tier pinning state. ''tentative'' (default for ingested) means NOT eligible for re-broadcast; ''pinned'' means corroborated by ≥2 local experiences via §10.5 REM audit. §10.3 contradiction detection forces tier=''tentative'' on both sides.';

-- ---------------------------------------------------------------------------
-- (2) lessons.tier — §10.6 mirror on locally-generated lessons.
...
-- a single column across both swarm-imported and local lessons uniformly.
-- ---------------------------------------------------------------------------
ALTER TABLE lessons
...
COMMENT ON COLUMN lessons.tier IS
  'SWARM_SPEC §10.6 two-tier pinning state. Locally-generated lessons default ''pinned'' (Tier-A by construction). §10.3 contradiction detection can demote to ''tentative'' if a swarm-ingested lesson is found to contradict it in a future cross-source scan.';

-- ---------------------------------------------------------------------------
-- (3) swarm_lesson_contradictions — receiver-side §10.3 edges.
...
COMMENT ON TABLE swarm_lesson_contradictions IS
  'SWARM_SPEC §10.3 Contradicts-trigger edges between two swarm lessons. ConscienceAgent populates this on ingest when cosine > 0.85 AND polarity inversion detected. Both endpoints are forced to swarm_lessons.tier=''tentative''. Resolution comes from new evidence, operator decision, or §10.5 REM falsification — never auto-resolved by 4g alone.';

-- ---------------------------------------------------------------------------
-- (4) chain_swarm_lesson_contradicts — idempotent upsert + tier demotion.
...
-- to tier='tentative' atomically with the edge insert (if either was
-- already 'pinned' via a future 4h promotion, this contradicts demotion
-- wins per §10.3 "Neither lesson is eligible for §10.6 Tier-A pinning
...
BEGIN
  ...
  SELECT tier INTO v_prior_a FROM swarm_lessons WHERE id = v_a_id;
  SELECT tier INTO v_prior_b FROM swarm_lessons WHERE id = v_b_id;
  ...
  -- Tier demotion: §10.3 "MUST mark both as tentative". This is
  -- idempotent — already-tentative lessons stay tentative.
  UPDATE swarm_lessons SET tier = 'tentative' WHERE id IN (v_a_id, v_b_id);
  ...
  RETURN jsonb_build_object(
    ...
    'prior_tier_a',      v_prior_a,
    'prior_tier_b',      v_prior_b,
    'demoted',           (v_prior_a = 'pinned' OR v_prior_b = 'pinned')
  );
END;
$$;
